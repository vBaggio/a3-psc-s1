...
Implementação da camada de relatórios analíticos com `RelatorioController` stateless e `RelatorioPanel` com três abas — Resumo Global, Desempenho por Projeto e Carga de Trabalho — todas com carregamento assíncrono via `SwingWorker` e DTOs imutáveis como contrato entre Controller e View.
...
Melhorias transversais de UX seguidas do redesign do módulo de projetos e tarefas: criação do `GestaoProjetoPanel` como tela unificada com save atômico (projeto + tarefas persistidos em um único `Salvar`), remoção do `TarefaPanel` autônomo, grade do `HomePanel` reorganizada para `3 + 2` e seed de dados com três usuários padrão (um por perfil) e uma equipe criados automaticamente via migration.

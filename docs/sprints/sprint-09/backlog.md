...
| ID | Descrição da Tarefa | Status |
|----|----------------------|--------|
| **TSK-15** | `HomePanel.criarGrade()` — substituir `XxxPanel::new` por lambdas `() -> new XxxPanel(usuario)`. Filtrar cards por perfil: COLABORADOR vê apenas `Projetos`; GERENTE vê `Projetos` e `Relatórios`; ADMIN vê todos os 5 cards. | ✅ Concluído |
...
| **FIX-08** | `HomePanel` — GERENTE visualizava Cargos, Usuários e Equipes (cadastros), o que não estava previsto na spec. Correção: cards de cadastro passam a exigir `perfil == ADMINISTRADOR`; Relatórios mantém `perfil != COLABORADOR` (ADMIN e GERENTE). GERENTE fica com acesso apenas a Projetos e Relatórios. | ✅ Concluído |
| **FIX-09** | `TarefaController` — `IllegalArgumentException: Tarefa não encontrada` ao criar tarefa com status `!= PENDENTE`. `atualizarStatus` chamado logo após `criarTarefa` usava `buscarTarefaOuFalhar` com EM próprio, que não enxerga entidades ainda não flushed na transação corrente. Correção: `buscarTarefaOuFalhar` aceita `EntityManager` opcional; com EM externo usa `em.find()` (enxerga o persistence context da transação). Todos os métodos com EM externo no controller atualizados. | ✅ Concluído |
